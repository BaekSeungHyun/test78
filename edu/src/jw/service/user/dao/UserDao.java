...
public class UserDao  extends AbstractDao{
	...
	public void getUser(UserVO userVO){
		...
		try {
			...
			if(rs.next()){
				System.out.println("1. db에서 확인 한 id, pwd==>:"+userVO.getId()+" : "+userVO.getPwd());
				userVO.setActive(true);
			}else{
				System.out.println("dbdp <"+userVO.getId()+">에 해당하는 자료가 없습니다. ");
			}
		}catch(Exception e){
			e.printStackTrace();
		} finally {
	...
	public List<UserVO> getUserList(){
		...
		try{
			...
			//==> :: 각각의 회원정보를 갖는 UserVO를 ArrayList에 저장
			while (rs.next()) {
